...
export const RatingModal = ({ setShowRatingModal, showRatingModal, counterRating, score, setCounterRating, setScore }) => {
    ...
    const onSubmit = (data) => {
        ...
        if (score === 0.0 && counterRating === 0) {
            ...
            axios
                .put(`http://localhost:4000/students/updateScoreRating/${courseMaterial[0]?.course_id}`, { score: rating, counter_rating: newCounterRating })
                .then((res) => {
                    setScore(rating)
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <Modal.Body className='d-flex-column align-items-center justify-content-center'>
                    ...
                    <div className="ratingStars">
                        <label className="starLabel" htmlFor="radio1">★</label>
                        <input
                            id="radio1"
                            type="radio"
                            {...register("rating")}
                            value="5"
                        />
                        <label className="starLabel" htmlFor="radio2">★</label>
                        <input
                            id="radio2"
                            type="radio"
                            {...register("rating")}
                            value="4"
                        />
                        <label className="starLabel" htmlFor="radio3">★</label>
                        <input
                            id="radio3"
                            type="radio"
                            {...register("rating")}
                            value="3"
                        />
                        <label className="starLabel" htmlFor="radio4">★</label>
                        <input
                            id="radio4"
                            type="radio"
                            {...register("rating")}
                            value="2"
                        />
                        <label className="starLabel" htmlFor="radio5">★</label>
                        <input
                            id="radio5"
                            type="radio"
                            {...register("rating")}
                    ...
                    <button className="btnNormal" type="submit">Puntúanos</button>
                </Modal.Body>
            </form>
        </Modal >
